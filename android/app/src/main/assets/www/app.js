...
function renderBooksView() {
  ...
  $('#bookSelectVisibleBtn')?.addEventListener('click', () => {
    candidates.map((poem) => poem.id).forEach((id) => state.bookSelectedIds.add(id));
    render();
  });
  ...
  $$('[data-select-book-candidate]').forEach((input) => {
    const updateBookCandidateSelection = (event) => {
    ...
    input.addEventListener('click', updateBookCandidateSelection);
    input.addEventListener('change', updateBookCandidateSelection);
  });
  $$('[data-candidate-row-id]').forEach((row) => row.addEventListener('click', (event) => {
  ...
  $$('[data-add-to-book]').forEach((btn) => btn.addEventListener('click', () => addToBook([btn.dataset.addToBook])));
  ...
  $$('[data-read-book-poem]').forEach((btn) => btn.addEventListener('click', (event) => { event.stopPropagation(); openReader(btn.dataset.readBookPoem, false); }));
...
function bindEvents() {
  ...
  els.closeMenuBtn.addEventListener('click', () => els.sidebar.classList.remove('open'));
  ...
  els.backupImportInput.addEventListener('click', (event) => {
  ...
  els.backupImportInput.addEventListener('change', async () => {
    const files = [...els.backupImportInput.files];
    if (files.length) {
      const items = await Promise.all(files.map(async (file) => ({ name: file.name, content: await file.text() })));
      await importJsonPayloads(items, 'yedekten');
    }
